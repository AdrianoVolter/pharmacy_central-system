  ...
  return (
    <Navbar bg="warning" variant="dark" className="p-1 ">
      ...
      <Nav className="me-auto my-2 my-sm-0 d-block">
        <Link className="nav-item btn p-1" to="/">
          {" "}
          <BiLogIn className=" link" /> Login
        </Link>
        {disabled ? null : (
          <Link className="nav-item btn p-1" to="/farmacias" disabled={true}>
            <BiStore className="nav-item link" /> Farmácias
          </Link>
        )}
        {disabled ? null : (
          <Link
            ...
            disabled={false}
          >
            <BiInjection className="link" /> Medicamentos
          </Link>
        )}
      </Nav>
    </Navbar>
